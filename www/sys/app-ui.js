...
(function() {
  ...
  $.fn
      ...
      .extend({
        ...
        'animateCss' : function(arg0, endCmd, cx) {

          cx = cx || {
            'arg0' : arg0
          };
          var rest = [], animationName;

          if (_.isArray(arg0) && arg0.length > 0) {
            animationName = arg0[0];
            rest = arg0.slice(1, arg0.length);
          } else {
            animationName = arg0;
          }
          ...
          function() {
            ...
            function _next() {
              ...
              if (rest.length > 0 && !this$.hasClass('stopped')) {

                this$.animateCss(rest, endCmd);
                return;

              }

              if (endCmd == 'loop') {
                this$.animateCss(cx.arg0, endCmd);
                return;
              }
              ...
              if (_.isFunction(endCmd)) {
                endCmd.apply(this, arguments);
              }
  ...
  // ELEMENT FUN
  //
  function createElementFun(ele, firstClass) {
  ...
  APP_ui.div = createElementFun('div', true);

  APP_ui.span = createElementFun('span');
  APP_ui.cite = createElementFun('cite');
  APP_ui.pre = createElementFun('pre');
  APP_ui.i = createElementFun('i');
  APP_ui.a = createElementFun('a');

  APP_ui.nav = createElementFun('nav', true);

  APP_ui.ul = createElementFun('ul', true);
  APP_ui.ol = createElementFun('ol', true);
  APP_ui.li = createElementFun('li');

  APP_ui.img = createElementFun('img', true);
  ...
  APP_ui.table = createElementFun('table', true);
  APP_ui.thead = createElementFun('thead', true);
  APP_ui.tbody = createElementFun('tbody', true);
  APP_ui.tr = createElementFun('tr', true);
  APP_ui.td = createElementFun('td');

  APP_ui.form = createElementFun('form', true);
  APP_ui._input = createElementFun('input', true);
  APP_ui.textarea = createElementFun('textarea', true);
  APP_ui.select = createElementFun('select', true);
  ...
  function inputUi(settings) {
    ...
    var isNumeric, ui, view$;

    settings.type = settings.type || settings.uiType || 'text';
    isNumeric = settings.type === 'numeric';
    // find type
    if (settings.type == 'string' || settings.type == 'numeric'
        || settings.type == 'input') {
      settings.type = 'text';
    }

    ui = APP_ui.templateUi();
    view$ = APP_ui.input(_.pick(settings, 'type', 'class', 'value', 'name',
        'text', 'html', 'id'));
    ui.view(view$);
    ...
    ui.editable(settings.editable);

    return ui;

    function action(arg0) {
      if (_.isFunction(arg0)) {
        view$.keypress(APP_ui.createReturnAction(arg0));
      }
      return ui;
    }
    ...
    function update(arg0) {
      if (_.isObject(arg0)) {
        if (_.isFunction(arg0.list)) {
          ...
          rQ.call(arg0.serviceId, arg0.parameters || {}, function(data) {
            fill(rQ.toList(data));
          });
        }
      }
  ...
  function link(label, callback) {
    return a$ = APP_ui.a(label, 'app-ui-link').click(callback);
  }
  APP_ui.link = link;
   ...
   * @memberOf APP_ui
   */
  function toggleLink(linkList) {
    var view$, switchSupport, current = 0;

    switchSupport = APP_ui.switchSupport();

    view$ = APP_ui.div('toggle-link');
    _.each(linkList, function(link$, i) {
      view$.append(link$);
      link$.click(next);
      switchSupport.put('' + i, link$);
    });

    switchSupport.show('' + current);

    return view$;

    function next() {
      current = (current + 1) % linkList.length;
      switchSupport.show('' + current);
    }
  }
  APP_ui.toggleLink = toggleLink;
   ...
   * @memberOf APP_ui
   */
  function input(arg0, arg1) {
    var v$ = APP_ui._input(arg0, arg1);
    if (_.isFunction(arg0)) {
      v$.keypress(createReturnAction(actionCb));
    }
    return v$;
  }
  APP_ui.input = input;

  function createReturnAction(actionCb) {
    return function(e) {
      var code;
      if (e) {
  ...
  APP_ui.createReturnAction = createReturnAction;
  ...
  function resultUi(settings) {
      ...
      if (!settings.onlyProcessLog) {
        if (sd.header && sd.table) {
          $.each(sd.header, function(i, head) {
            r$.append(gs.cell().text(head).addClass('col-' + (i % 2)));
          });
          $.each(sd.table, function(rowIndex, row) {
            r$ = gs.row().appendTo(t$).addClass('result-row');
            $.each(row, function(i, e) {
              r$.append(gs.cell().text(e).addClass('col-' + (i % 2)).addClass(
                  'row-' + (rowIndex % 2)));
            });
          });
        }
      ...
      if (_.isObject(processLog)) {
        ...
        if (lines) {
          $.each(lines, function(i, line) {
            var message, ui;
            r$ = gs.row().addClass('process-log-line').addClass(line.state)
                .appendTo(t$);
            // message
            // :
            ...
            if (_.isObject(message)) {
              ...
              r$.append(gs.cell().text(line.message || line.msg).addClass(
                  'message'));
            }
            ...
            if (_.isNumber(lastTime)) {
              r$
                  .append(gs.cell().text(
                      (lastTime - (+line.time)) + ':' + line.time).addClass(
                      'time'));
            } else {
              r$.append(gs.cell().text(APP_base.unixTimeToIso(line.time))
                  .addClass('time'));
            }
            lastTime = +line.time;
          });
        ...
        view$.append(gs.div().text('No process log').addClass('message'));
      }
    }
    ...
    function checkResult(str) {
      ...
      try {
        json = JSON.parse(str);
        if (_.isObject(json) && (_.isArray(json.table) || json.processLog)) {
          return json;
        }
      } catch (e) {
  ...
  function buttonUi(settings, callback) {
    ...
    label = _.isString(settings) ? settings
        : (settings.text || settings.label || '-no button label-');

    button$.text(label);

    _callback = callback || settings.click || settings.onClick;

    eFun = settings.buttonType != 'link' && _.isFunction(button$.button);
    ...
    function label(arg0) {
      if (!_.isUndefined(arg0)) {
        if (eFun) {
          button$.button('option', 'label', arg0);
        } else {
          button$.text(arg0);
        }
      }
      return eFun ? button$.button('option', 'label') : button$.text();
    }
  ...
  function parameterUi(parameterUiDef) {
    ...
    $.each(parameterUiDef, function(i, uiDef) {
      ...
      div$ = $('<div>').addClass('field').addClass('field-' + name);
      // ui
      ...
      // editable
      if (_.isBoolean(uiDef.editable) && _.isFunction(uiDef.ui.editable)) {
  ...
  function accordionUi(settings) {
    ...
    function add(title, content, showCb, hideCb) {
      var head$, body$, i;
      head$ = $('<div>').addClass('head').addClass('ui-widget-header').text(
          title);
  ...
  function templateUi(settings) {
    ...
    var ui, view$;
    var name, id, cx, map;
    var selectCb, doneCb, actionCb, changeCb, labelHolder;

    name = settings.name;

    view$ = settings.view$ || settings.view || APP_ui.div();
    ...
    ui = {
      ...
      'select' : function(arg0) {
        if (_.isFunction(arg0)) {
          selectCb = arg0;
        } else if (_.isFunction(selectCb)) {
          selectCb.apply(this, arguments);
        }
        return ui;
      },
      'done' : function(arg0) {
    ...
    function get(key) {
      return mp ? mp[key] : undefined;
    }

    function change(arg0) {
      if (_.isFunction(arg0)) {
        changeCb = arg0;
        if (view$ && _.isFunction(view$.change)) {
          view$.change(changeCb);
        }
      } else {
    ...
    function editable(arg0) {
      return APP_ui.handleDisabledAttr(view$, arg0);
    }
  ...
  function processLogUi(settings) {
    ...
    function init(serviceData) {
      ...
      $.each(lines, function(index, line) {
        ...
        $('<td>', {
          'class' : state
        }).css(td1Css).html(APP_base.toIsoDateTime(time)).appendTo(trElement);
        $('<td>', {
          'class' : state
  ...
  function fileListProcessLogUi(settings) {
    ...
    processLogList$ = $('<div>').addClass('process-log-list').appendTo(view$);
    ...
    function data(arg0) {
      ...
      if (_.isObject(arg0)) {
        ...
        $.each(list, function(i, row) {
          var row$ = $('<div>').addClass('process-log-list').appendTo(
              processLogList$);
  ...
  function videoUi(settings) {
    ...
    video$.on('webkitfullscreenchange mozfullscreenchange fullscreenchange',
        function(e) {
  ...
  function userMessagesUi(settings) {
    ...
    var ui = APP_ui.templateUi(), view$ = ui.view()
        .addClass('user-messages-ui'), currentList = [];
    ...
    function data(arg0) {
      ...
      if (currentList.length === 0) {
        ...
        $.each(currentList, function(i, line) {
          view$.append($('<div>').text(line.message).addClass(line.level));
        });
      }
    }
  ...
  function fileUploadUi(settings) {
    ...
    function() {
      ...
      function cb(data) {
        ...
        message$.empty().text('upload done for ' + list.length + ' file(s).')
            .show();
  ...
  function gridSupport(type) {
    ...
    function table(arg0) {
      arg0 = _.isString(arg0) ? arg0 : '';
      if (type === 'div') {
        return $('<div>').addClass('grid-table').addClass(arg0);
      }
      return $('<table>').addClass(arg0);
    }

    function row(arg0) {
      arg0 = _.isString(arg0) ? arg0 : '';
      if (type === 'div') {
        return $('<div>').addClass('grid-row').addClass(arg0);
      }
      return $('<tr>').addClass(arg0);
    }

    function cell(arg0) {
      arg0 = _.isString(arg0) ? arg0 : '';
      if (type === 'div') {
        return $('<div>').addClass('grid-cell').addClass(arg0);
      }
      return $('<td>').addClass(arg0);
    }
  ...
  function divUi(settings) {
    settings = settings || {};
    var ui = APP_ui.templateUi();
    var formatterFn = APP_base.findFunction(settings.formatter, APP_base.echo);
    var currentValue;
   ...
   * 
   * 'test' is a callback with a parameter file that expects true or false (true =>
   * continue upload).
   */
  /**
   * @memberOf APP_ui
  ...
  function dndDocumentUpload(holder$, settings) {
    ...
    function processUpload(files, event) {
      ...
      for (i = 0; i < files2.length; i++) {
        ...
        xhr.upload.addEventListener("progress", createProgressFn(file, i,
            progress), false);
  ...
  function fileListAreaUi(settings) {
    ...
    function footerRenderer(fileContainer$, footer$, file) {
      var delete$, actionPanel$ = $('<div>').addClass('file-action-panel');
      // DELETE BUTTON
      delete$ = APP_ui.link('delete', function() {
        if (settings.removeFileServiceId) {
          rQ.call(settings.removeFileServiceId, {
            'fileListTid' : fileListTid,
            'fileTid' : file.fileTid
          }, function(serviceData) {
            fileAreaUi.message('File ' + file.filename + ' removed.');
          });
        }
  ...
  function downloadFile(file) {
    ...
    var a$ = $('<a>', {
      'href' : url
    }).text('downloading file ' + file.filename + ' (' + file.fileTid + ')')
        .addClass('download-file').appendTo($('body'));
  ...
  function userMessageUi(settings) {
    ...
    actionName = _.isNumber(settings.actionName) ? settings.actionName : 'fade';
  ...
  function selectBoxesUi(settings) {
    ...
    if (_.isArray(selectList)) {
      $.each(selectList, function(i, e) {
        ...
        byValue[value] = v$;
        view$.append($('<div>').addClass('select-boxes-holder').append(v$,
            $('<span>').addClass('select-boxes-label').text(label)));
  ...
  function reportsListUi(settings) {
    ...
    $.each(defs, function(i, psrDef) {
      ...
      var title = psrDef.title || psrDef.serviceId || "Report -no title-";
    ...
    function _createUi(menu, level, parentPath) {
      ...
      if (_.isArray(menu)) {
        ...
        if (_.isArray(menu.submenu) && menu.submenu.length > 0) {
          ...
          ui.nodelistUi(_createUi(menu.submenu, level + 1, parentPath));
        } else {
          enhanceTop(ui, menu, settings.topRenderer);
  ...
  function stacked2Ui(settings) {
    ...
    function createHolderUi(ui) {
      ...
      if (_.isFunction(ui.headerUi)) {
        ...
        headerUi.view().append(APP_ui.div().addClass('title').text(ui.label()));
      }
      ...
      hui.view().append(headerUi.view(), bodyUi.view()).addClass('holder')
          .addClass('stack-level-' + size());
      hui.show = function() {
  ...
  function secvalUi(settings) {
    ...
    function value(arg0) {
      ...
      if (currentSecval.encValue.length > 0 || currentSecval.isEmpty === true) {
  ...
  function secvalAdminUi(settings) {
    ...
    function refreshView() {
      // read keys
      var nameList = [];
      $.each(localStorage, function(name, v) {
        var parts = name.split(APP_ui.secvalGlobal.lsPrefixKey);
        if (_.isString(parts[1])) {
          v = APP_base.ls(APP_ui.secvalGlobal.lsPrefixKey + parts[1]);
          nameList.push({
            'name' : parts[1],
            'key' : v
          });
      ...
      $.each(nameList, function(i, e) {
        ...
        var save$ = APP_ui.button('save', function() {
          APP_base.ls(APP_ui.secvalGlobal.lsPrefixKey + e.name, input$.val());
          refreshView();
        });
        var delete$ = APP_ui.button('delete', function() {
          APP_base.ls(APP_ui.secvalGlobal.lsPrefixKey + e.name, null);
          refreshView();
        });
    ...
    function addNew() {
      ...
      var add$ = APP_ui.button('add', function() {
        APP_base.ls(APP_ui.secvalGlobal.lsPrefixKey + inputKey$.val(), input$
            .val());
        refreshView();
      });

      table$.append($('<tr>').addClass('new').append(td1$.append(inputKey$),
          td2$.append(input$), $('<td>').append(add$)));
    }
  ...
  //
  // HOUR MINUTE UI -begin-
  //

  function hourMinuteUi(settings) {

    settings = settings || {};

    var ui, view$, hour$, min$;
    var valueType = settings.valueType || 'hour';

    ui = APP_ui.templateUi();

    view$ = ui.view().addClass('hour-minute-ui');
    ...
    view$.append(hour$, APP_ui.div(':', 'separator'), min$);
    ...
    function value(arg0) {
      ...
      if (!_.isUndefined(arg0)) {
        ...
        if (_.isFinite(val)) {
          if ('hour' == valueType) {
            min = Math.floor((val * 60 + 0.5) % 60);
            hour = Math.floor((val * 60 + 0.5) / 60);
          }
          min$.val(settings.hideZeroValues && Math.abs(hour) < 1
              && Math.abs(min) < (1 / 60) ? '' : min);
          hour$.val(settings.hideZeroValues && Math.abs(hour) < 1 ? '' : hour);
        }
      }
      val = null;
      hour = +(hour$.val() || 0);
      ...
      if (_.isFinite(hour) && _.isFinite(min)) {
        if ('hour' == valueType) {
          val = hour + min / 60;
        }
      }
    ...
    function editable(arg0) {
      if (_.isBoolean(arg0)) {
        min$.prop('disabled', !arg0);
        hour$.prop('disabled', !arg0);
      }
      return !min$.prop('disabled');
    }

  }
  APP_ui.hourMinuteUi = hourMinuteUi;
  ...
  function booleanUi(settings) {
    ...
    function value(arg0) {
      ...
      if (arg0 !== undefined) {
        if (_.isString(arg0)) {
          ...
          if (arg0 === 'true' || arg0 === 't' || arg0 === 'yes' || arg0 === 'y'
              || arg0 === 'on') {
            bool = true;
          }
      ...
      return checkbox$ ? '' + checkbox$.prop('checked') : (span$ ? span$.text()
          : '');
    }
    ...
    function _value(bool) {
      ui.view().removeClass('true').removeClass('false').addClass('' + bool);
      if (checkbox$) {
  ...
  function fileAreaUi(settings) {
    ...
    var openFile = APP_base.findFunction(settings.openFile, APP_ui.openFile);
    ...
    dropFilesHere$ = APP_ui.div().text(
        settings.dropFileHereLabel || 'Please drop files here.').addClass(
        'file-area-ui-drop-files-here');
    // userMessageUi
        ...
        body$.sortable({
          ...
          update : function(event, ui) {
            ...
            if (_.isFunction(moveFileCb)) {
              ...
              moveFileCb.apply(this, [ list, fileInfoItem, newOrd ]);
            }
            checkDropFilesHere();
          },
    ...
    function addFile(index, fileInfo) {
      ...
      footer$.append($('<div>').addClass('filename').text(label).attr('title',
          label));
    ...
    function initDnd() {
      ...
      var settings = {
        ...
        'begin' : function(files, event) {
          ...
          userMessageUi.text('Start: ' + currentFiles.length + ' file(s).');
          ord = undefined;
          ...
          body$.find('div.drop-files-here').addClass('file-container-ondrag');
          userMessageUi.hide();
        ...
        'progress' : function(e, file, index) {
          ...
          userMessageUi.text('Uploading : ' + file.name + ' (' + (index + 1)
              + ' of ' + currentFiles.length + ')');
...
(function() {
  ...
  function anakapaViewUi(arg0, settings) {
    ...
    function forward(name, value, data) {
      ...
      // add action !!!
      if (uiDef.renderAvActions && uiDef.actions) {
        ttUi = APP_ui.renderAvActions(ttUi, uiDef);
      }
      // TODO move to tt3 ui
    ...
    function processAction(aEvent, skipConfirmation) {
      ...
      var initParameters = APP_base.findValue(action.initParameters || {},
          aEvent);
      var parameters = APP_base.findValue(action.parameters || {}, aEvent);
      ...
      serviceFnIntern = APP_base
          .findFunction(action.serviceFn, serviceFnIntern);

      // NEW END

      valueObject = $
          .extend({}, parameters0, initParameters, value, parameters);
    ...
    function confirmationUi(html, yesCb, noCb) {
      ...
      buttons$.append(
          APP_ui.button(L('yes'), yesCb).addClass('label-name-yes'), APP_ui
              .button(L('no'), noCb).addClass('label-name-no'));
  ...
  function renderAvActions(ui, settings) {
    ...
    var gs, view$, buttonPanel$, messageUi, iValue, iData;
    var currentValue;

    gs = APP_ui.gridSupport('div');
    view$ = ui.view();
    buttonPanel$ = gs.div().addClass('detail-buttons').hide();
    messageUi = APP_ui.templateUi();
    view$.append(buttonPanel$, messageUi.view());
    ...
    function data() {
      return iData.apply(ui, arguments);
    }
    ...
    function buildActionPanel() {
      ...
      $.each(settings.actions, function(i, action) {
        ...
        if (APP_ui.isActionVisible(action, cx)) {
          ...
          label = L(action);
          actionUiFn = APP_base
              .findFunction(action.uiType || 'APP_ui.buttonUi');
            ...
            fireAction({
              ...
              'callback' : function(data) {
                ...
                if (messageUi.size() > 0) {
                  ...
                  messageUi.data('\'' + action.name + '\' done.');
                }
                return true;
              }
            });
  ...
  APP_ui.renderAvActions = renderAvActions;
  ...
  function fieldsUi(settings) {
    ...
    $.each(uiSections.list(), function(i, fields) {
      ...
      $.each(fields, function(index, field) {
        ...
        tr$.append(td$ = gs.cell().addClass('field-name field-name-' + name)
            .text(label));
        APP_ui.addTooltip(td$, descr);
        tr$.append(gs.cell().addClass('field-value field-value-' + name)
            .append(ui.view()));
    ...
    function value(data) {
      ...
      if (_.isObject(value)) {
        ...
        $.each(fieldsui.cxMap, function(name, e) {
          ...
          visible = APP_base.bool(APP_base.findValue(e.field.visible, cx));
  ...
  function tableUi(settings) {
    ...
    function init() {
      ...
      $.each(fields, function(index, field) {
        ...
        td$.addClass('field-name field-name-' + field.name + ' field-column '
            + 'field-column-' + field.name + ' visible-' + visible);
        headerTr$.append(td$);
    ...
    function actionsCellRenderer(actions) {
      ...
      function commandsCellRenderer(td$, rowIndex, colIndex, row, field) {
        var div$ = APP_ui.div().addClass('row-actions field-column').appendTo(
            td$);
        var actionCounter = 0;
        var name = field.name;
        $.each(actions, function(i, action) {
              ...
              fireAction({
                ...
                'callback' : function(data) {
                  ...
                  if (action.type === 'rowUpdate') {
                    if (list.length > 0) {
                      populateRow(td$, rowIndex, name, list[0]);
                    }
                    return false;
                  }
                  return true;
    ...
    function populateData(data, trIn$, rowIndexIn, nameIn, rowIn) {
      ...
      $.each(fields, function(i, field) {
        ...
        cellRendererFn = APP_base.findFunction(field.cellRenderer,
            createDefaultCellRenderer(field));
        cellRendererFns[field.name] = function(td$, rowIndex, colIndex) {
      ...
      if (rowIn) {
        ...
        for (j = 0; j < columnNames.length; j++) {
          ...
          cellRenderer(td$, rowIndexIn, j, rowIn, fields[j], rowEditors);
        }
      } else {
        dataTable$.empty();
      ...
      function defaultRowRenderer(tr$, rowIndex, row, rowEditors) {
        ...
        tr$.click(function(event) {
          ...
          if (rowSelectAction) {
            logger.debug('tableui : start fireAction with rowSelectAction');
            fireAction({
              'event' : event,
      ...
      function createDefaultCellRenderer(colDef) {
        ...
        function defaultCellRenderer(td$, rowIndex, colIndex, row, fieldDef,
            rowEditorList) {
          ...
          uidef.editable = APP_base.findValue(fieldDef.editable || false, cx);
          var visible = APP_base.bool(APP_base.findValue(fieldDef.visible, cx));
          td$.addClass('visible-' + visible).addClass(
              'field-value field-value-' + name + ' field-column '
                  + 'field-column-' + name).addClass(
              'type-' + (uidef.editable ? 'edit' : 'view'));
    ...
    function filterValues() {
      ...
      $.each(filterEditors, function(index, filterUi) {
        ...
        var value = ui.value();
        if (filterUi.filter === 'startWith' && filterUi.type !== 'numeric') {
          value = (value || '') + '%';
        }
        if (value) {
  ...
  function littleServiceDataUi(settings) {
    ...
    function data(serviceData) {
      ...
      _.each(header, function(h, i) {
        tr$.append(gs.cell().addClass('field-column field-column-' + h).text(
            L(h)));
      });
      //
  ...
  function downloadFileCellRenderer(td$, rowIndex, colIndex, row, fieldDef,
    ...
    td$.addClass('visible-' + visible).addClass(
        'field-value field-value-' + name + ' field-column ' + 'field-column-'
            + name);
