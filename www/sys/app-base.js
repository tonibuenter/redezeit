...
(function() {
   ...
   * 
   */
  function dispatchInit(settings) {

    settings = settings || {};

    var isFile, isCordova;
    ...
    isCordova = !APP_base.CORDOVA_OFF;

    if (isFile && isCordova) {
      (settings.phone ? settings.phone : _.noop)();
    }
    if (isFile && !isCordova) {
      (settings.offline ? settings.offline : _.noop)();
    }
    if (!isFile && !isCordova) {
      (settings.web ? settings.web : _.noop)();
    }
    if (!isFile && isCordova) {
      (settings.webPlus ? settings.webPlus : _.noop)();
    }
    //     
    //
    // else {
  ...
  APP_base.dispatchInit = dispatchInit;

  function partition(sum, weights) {
    var wSum, dec = sum, part = [], v, w;

    wSum = _.reduce(weights, function(memo, num) {
      return memo + num;
    }, 0);
    if (wSum == 0) {
      return;
